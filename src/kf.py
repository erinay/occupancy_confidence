...
def on_key(event):
    ...
    masked_decay = np.where(visibility_t1, Confidence_values_conv, 0).astype(np.float32)
    
    # Optical Flow
    normed_buffered_conv = buffered_binary_conv/3
    flow = cv2.calcOpticalFlowFarneback(masked_decay, normed_buffered_conv, None, 0.5, 3, 15, 3, 5, 1.2, 0)
    decay_map = decay_masked(normed_buffered_conv, Confidence_values_decay, visibility_t1, None, decay_rate=0.7)

    # visualize_decay_masked(normed_buffered_conv, Confidence_values_decay, visibility_t1, None, decay_rate=0.7)
    ...
    axs[1].set_title(f"Decay Map at Frame \nFrame {frame_index}")
    im2.set_data(masked_decay)
    axs[2].set_title("Masked decay Map")
